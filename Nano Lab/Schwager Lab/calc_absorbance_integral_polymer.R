...
folder <- 'C:\\ <---- path to measuerment ----> \\measuerment data' # put your folder desitination hier where all the xlsx from lab class are stored
...
aliasmeasdata <- 'absorbance_integral_polymer'  ## name alias 
...
plott9 <- ggplot(alldata4[[measdata]]) + 
  ...
  annotate("text", x = x1+ 70, y = 0.2, label = paste("sum(ODdv)= ", ODsum, " THz", sep=""), size = 5) +
  ggtitle(titel)
print(plott9)
dev.off()
