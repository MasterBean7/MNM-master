...
folder <- 'C:\\<-----  your path to data in csv format ---->\\measuerment data' # put your folder desitination hier where all the xlsx from lab class are stored
...
aliasmeasdata <- 'monomer_absorption'  ## name alias 
...
plott9 <- ggplot(alldata4[[measdata]]) + 
  ...
  annotate("text", x = x1+40, y = halfmax, label = paste(x1, " nm", sep=""), size = 5) +
  annotate("text", x = x2-40, y = halfmax, label = paste(x2, " nm", sep=""), size = 5) +
  annotate("text", x = max[["x"]]+70, y = max[["y"]], label = paste("max = ", max[["y"]], " Abs at ",max[["x"]]," nm", sep=""), size = 5) +
  ggtitle(titel)
print(plott9)
dev.off()
